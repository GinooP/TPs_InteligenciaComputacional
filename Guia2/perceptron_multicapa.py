...
class Capa:
    def __init__(self, W, y, delta):
        ...
        self.delta = delta # Vector del gradiente del error local instantaneo de cada neurona
...
def generar_perceptron_multicapa(capas, patrones, eta, epocas, tol,winout):
    ...
    W = np.array(rng.random((capas[0], M)) - 0.5) #  Setear los pesos entre [-0.5 0.5] del vector de pesos sinápticos
    vector_capas.append(Capa(W, [], []))
    for i in range(1,cant_capas):
        W = np.array(rng.random((capas[i], capas[i-1]+1)) - 0.5) #  Setear los pesos entre [-0.5 0.5] del vector de pesos sinápticos
        vector_capas.append(Capa(W, [], []))
    ...
    for i in range(epocas):
        ...
        for j in range(N):
            ...
            if(winout): # a esta parte la podriamos hacer con un puntero a funcion o algo por el estilo
