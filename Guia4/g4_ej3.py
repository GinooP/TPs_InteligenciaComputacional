...
entradas = patrones_trn[:, 0:4] # Usamos solo las 4 características
salidas = patrones_trn[:, 4:7] # Usamos solo las 4 características

K_values = range(2,31)
epocas = 100

# Usar una lista es más seguro y fácil en Python para ir guardando los resultados
compactitudes = [] 

# 2. Iteración sobre los valores de K
for k in K_values:
    # Asegúrate de pasar 'entradas' para no incluir las columnas de las etiquetas
    K_centroides, lotes_patrones = km.k_means(entradas, k, epocas)

    # Asumo que tu función necesita los centroides (K_centroides) para calcular las distancias,
    # si realmente recibe el escalar 'k', puedes cambiarlo de vuelta a 'k'.
